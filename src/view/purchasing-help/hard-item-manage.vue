<template>
  <div class="hard-iem-manage">
    <Card>
      ...
      <Table :data="hardList" :columns="hardColumns" stripe :highlight-row="true" ref="tables" height="480" width="100%"
        ...
        <Form ref="itemsForm" :model="itemForm" :rules="ruleValidate_Item" :label-width="100">
          ...
          <FormItem label="硬件图片:">
            ...
            <Upload ref="upload" :show-upload-list="false" :on-success="handleSuccess" :default-file-list="defaultList"
              ...
              :format="['jpg','jpeg','png']" :max-size="2048" multiple type="drag" :action="actionUrl" style="display: inline-block;width:58px;">
    ...
    <Modal v-model="itemConfig_Modal" width="60%">
      ...
      <div>
        ...
        <Form ref="configformValidate" :label-width="120">
          ...
          <FormItem v-for="(item, index) in configData" :key="item.SerialNo" :label="item.ItmeName+'：'">
            <RadioGroup v-model="item.onSelect">
              <Radio v-for="(SubItem, index) in item.ConfigItme" :key="SubItem.SerialNo" :label="SubItem.SerialNo">{{SubItem.ItmeName}}</Radio>
            </RadioGroup>
          </FormItem>
        </Form>
      ...
      <div slot="footer">
        <Button type="primary" @click="handleConfigSubmit('configformValidate')" :loading="item_loading">保存</Button>
        <Button type="warning" @click="handleReset('configformValidate')" style="margin-left: 8px">重置</Button>
  ...
  export default {
    ...
    methods: {
      ...
      addItem() {
        ...
        this.onSelect.row = { //重置表单后 会清空默认值   临时处理：重新赋默认值
          IsStop: '0',
          IsEnergy: '1',
          WarrantyPeriod: 1,
          WarrantyCost: 1,
      ...
      setItem() {
        ...
        hardType.getTypeConfig(SerialNo).then(res => {
          ...
          this.titleName.name = '[' + rows.SubType + '-' + rows.ItemModel + ']配置硬件信息'
          ...
          if (ConfigDesc) {
            ...
            NewData = this.configData.map(item => {
              let onItem = ConfigDesc.filter(sub => sub.ParentNo === item.SerialNo);
              let onSelect = '';
              if (onItem.length > 0) {
            ...
            render: (h, params) => {
              return h(
                ...
                [
                  h("Icon", {
                      props: {
                        type: this.hardList[params.index].ConfigDesc.length > 0 ? 'ios-eye' : 'ios-settings',
                        size: this.hardList[params.index].ConfigDesc.length > 0 ? '24' : '24'
                      },
                      on: {
                        click: () => {
                          let SerialNo = params.row.TypeNo;
                          // this.configData=[];
